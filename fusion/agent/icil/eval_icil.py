...
def evaluate_on_env(model, device, env, num_eval_ep=10, render=False, max_test_ep_len=1000, image=False, model_est=None):
    ...
    with torch.no_grad():
        ...
        while count_done < num_eval_ep: 
            ...
            if image: 
                ...
                #            
                pred_state = model_est.get_state_pred(img_state)
                act_logits = model.policy_network(model.causal_feature_encoder(pred_state))                
            else: 
