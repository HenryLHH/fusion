...
from utils.logger import WandbLogger, DummyLogger
from utils.networks import MLPGaussianPerturbationActor, \
...
class BCQL(nn.Module):
    ...
    def act(self, obs, deterministic=False, with_logprob=False, batch_inputs=False):
        '''
...
class BCQLTrainer:
    ...
    def evaluate(self, eval_episodes):
        ...
        self.model.train()
        results = {}
        results["reward"] = epi_ret
        results["cost"] = epi_cost
        results["ep_len"] = epi_len
        results["success"] = np.mean(np.array(success)[:, 0])
        results["oor"] = np.mean(np.array(success)[:, 1])
        results["crash"] = np.mean(np.array(success)[:, 2])
        results["max_time"] = np.mean(np.array(success)[:, 3])
    ...
    def rollout(self, eval_episodes):
        ...
        count_ep = 0
        success = []
        while count_ep < eval_episodes: 
            act, _ = self.model.act(obs['state'], True, True, batch_inputs=True)
            obs_next, reward, done, info = self.env.step(act)
            cost = np.array([info[i]["cost_sparse"] for i in range(n_envs)])
            obs = obs_next
            ...
            for i in range(n_envs):
                if done[i]:
                    ...
                    pbar.update(1)
                    if count_ep >= eval_episodes: 
                        break
        pbar.close()
        return episode_ret/count_ep, episode_len/count_ep, episode_cost/count_ep, success
