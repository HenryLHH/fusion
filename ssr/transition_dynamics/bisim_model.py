...
def eval_loss_dataloader(model, criterion, dataloader, verbose=0):
    ...
    for data in dataloader:
        ...
        state_next_est, z, kl_loss, loss_bisim, loss_bisim_cost = model(img, state, action, reward, cost)
        state_next_est = state_next_est.detach()
        kl_loss = kl_loss.detach()
        loss_bisim = loss_bisim.detach()
        loss_bisim_cost = loss_bisim_cost.detach()
        z = z.detach()


        loss_state_est = criterion(state_next_est, state_next)  
        loss += (loss_state_est + loss_bisim_cost + kl_loss).item()
        # print('val loss: {:4f} | state est. loss: {:.4f} | bisim loss: {:.4f} | kl: {:.4f} '.format(loss.item(), loss_state_est.item(), loss_bisim.item(), kl_loss.item()), end='\r')
...
def visualize_ckpt(model, ckpt, dataloader):
    ...
    for data in dataloader:
        ...
        reward = CUDA(reward)
        cost = CUDA(cost)
        _, z, _, _, _ = model(img, state, action, reward, cost)
        print(z.shape)
...
class ImageStateEncoder(nn.Module):
    def __init__(self, hidden_dim=16, nc=3, output_dim=OBS_DIM):
        ...
        self.encoder = nn.ModuleList([build_encoder_net(hidden_dim, 1) for _ in range(nc)])
...
class BisimEncoder(nn.Module):
    def __init__(self, hidden_dim, output_dim, causal=False) -> None:
        ...
        self.output_dim = output_dim
        if causal:
            self.image_encoder = ImageStateEncoder(hidden_dim=hidden_dim, nc=5, output_dim=hidden_dim)
        else:
            self.image_encoder = ImageStateEncoder_NonCausal(hidden_dim=hidden_dim, nc=5, output_dim=hidden_dim)
        ...
        # Freeze the pretrained dynamics model
        p = list(self.state_dynamics.parameters())
        for v in p:
    ...
    def forward(self, img, state, action, reward, cost):
        ...
        z , mu_z, logvar_z = self.image_encoder(img)
        ...
        state_est = self.state_estimator(z)
        state_next_est, _ = self.state_dynamics(state_est, action)  
        _, _, kl_loss = kl_divergence(mu_z, logvar_z)
        ### task 2: cost bisimulation metrics
        z_2 = z[perm_idx]
        reward_2 = reward[perm_idx]
        cost_2 = cost[perm_idx].float()
        
        z_dist = F.smooth_l1_loss(z, z_2)
        r_dist = F.smooth_l1_loss(reward, reward_2)
        c_dist = F.smooth_l1_loss(cost.float(), cost_2)
        ...
        cost_bisimilarity = c_dist + 0.99 * trans_dist # + c_dist

        loss_bisim = (z_dist - bisimilarity).pow(2).mean()
        loss_bisim_cost = (z_dist - cost_bisimilarity).pow(2).mean()
        
        
        pred = self.state_classifier(z)
        loss_cls = self.criterion(pred, cost)

        return state_next_est, z, kl_loss, loss_cls, loss_bisim, loss_bisim_cost


class BisimEncoder_State(nn.Module):
    def __init__(self, hidden_dim, output_dim, causal=False) -> None:
        super().__init__()
        ...
        if causal:
            self.state_encoder = ImageStateEncoder(hidden_dim=hidden_dim, nc=5, output_dim=hidden_dim)
        else:
    ...
    def forward(self, img, state, action, reward, cost):
        ...
        return z.detach(), loss_cls, pred_cls
...
class BisimEncoder_Head(nn.Module):
    ...
    def _get_bisim_loss(self, z, img, state):
        pass
...
if __name__ == '__main__':
    ...
    if args.encoder == 'image':
        train_set = BisimDataset(file_path='../data/data_bisim_generalize_post', num_files=NUM_FILES//2) # TODO: //10
    ...
    # val_set_raw = BisimDataset(file_path='../envs/data_bisim_generalize_post', \
    #                                         num_files=NUM_FILES-NUM_FILES//10, offset=0*NUM_FILES//10, noise_scale=0.0)
    
    # test_set = BisimDataset(file_path='../VAE/data_bisim_post' , num_files=NUM_FILES, noise_scale=1.0)
    # test_set_raw = BisimDataset(file_path='../VAE/data_bisim_post' , num_files=NUM_FILES, noise_scale=0.0)
        ...
        if args.encoder == 'image':
            for epoch in range(NUM_EPOCHS):
                ...
                for data in train_dataloader:
                    ...
                    state_next_est, z, kl_loss, loss_cls, loss_bisim, loss_bisim_cost = model(img, state, action, reward, cost)
                    
                    loss_state_est = criterion(state_next_est, state_next)  
                    loss = loss_cls + loss_bisim_cost + 0.1*kl_loss # loss_state_est + 0.1*kl_loss + loss_bisim_cost

                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()
                    loss_train += loss.item()
                    print('train loss: {:4f} | state est. loss: {:.4f} | bisim loss: {:.4f} | kl: {:.4f} '.format(loss.item(), loss_state_est.item(), loss_bisim.item(), kl_loss.item()), end='\r')
            ...
            for epoch in range(NUM_EPOCHS):
                ...
                for data in train_dataloader:
                    ...
                    z, loss, pred_cls = model(img, state, action, reward, cost)
                    acc = len(torch.where(pred_cls==cost)[0]) / len(cost)
                    TP = len(torch.where(torch.logical_and(pred_cls==cost, cost))[0])
                    recall = TP / ((len(torch.where(cost)[0]))+1e-8)
                    precision = TP / (len(torch.where(pred_cls)[0])+1e-8)
