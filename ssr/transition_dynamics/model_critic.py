...
def eval_loss_dataloader_state(model, dataloader, verbose=0):
    ...
    for data in dataloader:
        ...
        loss += (loss_vr).item() # (loss_est+loss_ret+loss_bisim+loss_bisim_cost).item()
...
if __name__ == '__main__':
    ...
    if args.encoder in ['image', 'image_spurious']:
        model = BisimEncoder_Head_BP_Value(hidden_dim=64, output_dim=35, causal=args.causal)
        ...
        if args.encoder in ['image', 'image_spurious']:
            for epoch in range(NUM_EPOCHS):
                ...
                for data in train_dataloader:
                    ...
                    loss = loss_vr #  loss_est + #  loss_est + loss_act # + loss_cls + loss_bisim + loss_bisim_cost # loss_state_est + 0.1*kl_loss + loss_bisim_cost
