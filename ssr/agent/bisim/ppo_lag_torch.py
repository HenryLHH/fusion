...
from .model import CostValueNetwork, CostValueNetworkMixin
...
class PPOLagTorchPolicy(
    ...
    def loss(
        ...
        loss_bisim, loss_dynaimcs = model.get_bisim_loss(train_batch)
        ...
        total_loss += self.config["bisim_coef"] *(loss_bisim + loss_dynaimcs) # bisim
        ...
        model.tower_stats["bisim_loss"] = loss_bisim
        model.tower_stats["dynamics_loss"] = loss_dynaimcs
        ...
        return convert_to_numpy(
            {
                ...
                #########################
                "bisim_loss": torch.mean(
                    torch.stack(self.get_tower_stats("bisim_loss"))
                ),
                "dynamics_loss": torch.mean(
                    torch.stack(self.get_tower_stats("dynamics_loss"))
                ),
            }
        )
