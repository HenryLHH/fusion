...
class SafeDecisionTransformer_Structure(nn.Module):
    ...
    def forward(self, timesteps, states, actions, returns_to_go, returns_to_go_cost, deterministic=False):
        ...
        try: 
            ...
            torch.save(state, '1.pt')
            torch.save(self.embed_state.state_dict(), '2.pt')
            torch.save(time_embeddings, '3.pt')
            torch.save(timesteps, '4.pt')
...
class SafeDecisionTransformer_Structure_Dynamics(nn.Module):
    def __init__(self, state_dim, act_dim, n_blocks, h_dim, context_len,
                 n_heads, drop_p, max_timestep=4096):
        super().__init__()
