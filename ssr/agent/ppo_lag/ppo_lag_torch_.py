...
from ray.rllib.agents.ppo.ppo import validate_config as original_validate_config, PPOTrainer, \
    DEFAULT_CONFIG as ppo_default_config, warn_about_bad_reward_scales, UpdateKL, PPOTrainerTorch

from ray.rllib.policy.torch_mixins import setup_mixins, ValueNetworkMixin, KLCoeffMixin, EntropyCoeffSchedule, \
    PPOTorchPolicy, kl_and_loss_stats, LearningRateSchedule
...
def post_process_fn(policy: Policy,
    ...
    sample_batch = postprocess_ppo_gae(policy, sample_batch, other_agent_batches, episode) # compute_gae_for_sample_batch
    return sample_batch
...
def ppo_lag_surrogate_loss(
    ...
    surrogate_cost = cost_adv * torch.clamp(logp_ratio, 0., 1 + policy.config["clip_param"])
    mean_cost_loss = reduce_mean_valid(surrogate_cost)
    cost_value_loss = torch.square(model.get_cost_value() - train_batch[COST_TARGET])
    penalty = policy._penalty_active_fn(policy._penalty_param)
    ###################################
    ...
    if policy.config["use_gae"]:
        ...
        total_loss = reduce_mean_valid(
            ...
            policy.entropy_coeff * curr_entropy) + penalty * mean_cost_loss
...
def after_init(policy, obs_space, action_space, config):
    ...
    policy._penalty_active_fn = nn.Softplus()
    policy._penalty = policy._penalty_active_fn(penalty_param)
    policy._penalty_param = penalty_param
    print('init penalty: ', policy._penalty_param)
    ####################################



PPOLagPolicy = PPOTorchPolicy.with_updates(
    name="PPOLagPolicy",
    get_default_config=lambda: PPO_LAG_CONFIG,
    postprocess_fn=post_process_fn,
    loss_fn=ppo_lag_surrogate_loss,
    extra_grad_process_fn=gradient_fn, #gradients_fn
    stats_fn=new_stats,
    make_model=make_model,
    before_init=setup_mixins_ppo_lag,
...
PPOLag = PPOTrainerTorch.with_updates(
    name="PPOLag",
