...
from ssr.encoder.model_critic import BisimEncoder_Head_BP_Value
...
class TorchCustomModel(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        self.actor_model.load_state_dict(torch.load('/home/haohong/0_causal_drive/ssr-rl/checkpoint/attention_encoder_actor.pt', map_location="cpu"))
        
        self.value_model = BisimEncoder_Head_BP_Value(hidden_dim=self.hidden_dim, output_dim=self.output_dim, causal=True)
        self.value_model.load_state_dict(torch.load('/home/haohong/0_causal_drive/ssr-rl/checkpoint/attention_encoder_value.pt', map_location="cpu"))
        ...
        p = list(self.actor_model.parameters()) + list(self.value_model.parameters()) + list(self.value_cost_model.parameters()) 
        for v in p:
        ...
        p = list(self.actor_model.action_encoder.parameters()) + list(self.actor_model.action_estimator.parameters()) \
            ...
            + list(self.value_cost_model.value_encoder.parameters()) + list(self.value_cost_model.value_estimator.parameters())                
        for v in p:
