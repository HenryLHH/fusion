...
def eval_loss_dataloader_state(model, dataloader, verbose=0):
    ...
    for data in dataloader:
        ...
        loss += (loss_vc).item() # (loss_est+loss_ret+loss_bisim+loss_bisim_cost).item()
...
class BisimEncoder_Head_BP_Cost(nn.Module):
    def __init__(self, hidden_dim, output_dim, causal=False) -> None:
        super().__init__()
    ...
    def forward(self, img, lidar, state, action, reward, cost, vr_target, vc_target, deterministic=False):
        ...
        if self.causal:
            ...
            pred_vc = self.value_estimator(z_val)
            ...
            loss_bisim_r = torch.tensor(0.) # self._compute_bisim(z_reward, state, action, reward)
            loss_bisim_c = self._compute_bisim(z_val, state, action, cost)
...
class BisimEncoder_Head_BP_Cost_Causal(nn.Module):
    def __init__(self, hidden_dim, output_dim, causal=False) -> None:
        super().__init__()
...
if __name__ == '__main__':
    ...
    if args.encoder == 'image':
        ...
        val_set_noise = BisimDataset(file_path='/home/haohong/0_causal_drive/baselines_clean/data/data_bisim_generalize_post', \
    ...
    if args.encoder in ['image', 'image_spurious']:
        model = BisimEncoder_Head_BP_Cost(hidden_dim=64, output_dim=35, causal=args.causal)
        ...
        if args.encoder in ['image', 'image_spurious']:
            for epoch in range(NUM_EPOCHS):
                ...
                for data in train_dataloader:
                    ...
                    loss = loss_vc #  loss_est + #  loss_est + loss_act # + loss_cls + loss_bisim + loss_bisim_cost # loss_state_est + 0.1*kl_loss + loss_bisim_cost
